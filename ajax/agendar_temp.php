...
$query = $pdo->prepare("SELECT * FROM procedimentos where id = :servico");
...
$query = $pdo->prepare("SELECT * FROM dias where funcionario = :funcionario and (dia = '$dia_procurado' or data = '$data')");
$query->bindValue(":funcionario", "$funcionario");
$query->execute();
$res = $query->fetchAll(PDO::FETCH_ASSOC);
if(@count($res) == 0){
...
$query = $pdo->prepare("SELECT * FROM pacientes where cpf LIKE :cpf ");
$query->bindValue(":cpf", "$cpf");
$query->execute();
$res = $query->fetchAll(PDO::FETCH_ASSOC);
if(@count($res) == 0){
	$query = $pdo->prepare("INSERT INTO pacientes SET nome = :nome, telefone = :telefone, data_cad = curDate(),  senha = '$senha_crip', cpf = :cpf");
	...
	$query22 = $pdo->query("SELECT * FROM receber where cliente = '$id_cliente' and data_venc < curDate() and pago = 'Não'");
