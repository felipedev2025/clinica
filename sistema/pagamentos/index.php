...
<div id="info_pagamento" style="text-align: center;"> 
        <p class="h3 font-weight-normal" style=" font-size: 18px; border-radius: 4px;"><span>(Procedimento <?=$nome_servico;?>)</span> <span style="color:green; ">R$ <?=$valorF;?></span> </p>
        ...
        <?php if($porc_servico <= 0){ ?>
        <div style="margin-bottom: 8px; font-size: 13px"><a id="btn_clique" onclick="clique()" href="<?php echo $url_sistema ?>pagamentos/pagamento_aprovado.php?id_agd=<?php echo $id_conta ?>"><b><span id="clique_aqui">>>CLIQUE AQUI<<</span></b></a> para confirmar o agendamento e deixar para pagar no local</div>
        <?php } ?>
...
<div  id="paymentBrick_container">
    ...
    <script>
        ...
        var opcao_pagar = "<?=$opcao_pagar;?>";             
            if(opcao_pagar == 'Não'){               
                document.getElementById("btn_clique").click();
            }
    ...
    $.ajax(settings).done(function(response) {
        try {
            if (response.status == "pago") {
                ...
                if (redi.trim() == "Sim") {
                    setTimeout(() => {
                        window.location = "../painel_cliente";
                        //$("#btn_form").click();
                    }, 6000);
                }
            } else {
