...
$query = $pdo->query("SELECT * FROM agendamentos where ref_pix != '' and pago != 'Sim' and pgto_parcial is null");
...
//percorrer os agendamentos procurando agendamentos pendentes
...
if($ref_pix != "" ){
			...
			if($status_api == 'approved'){
				...
				if($valor_pago >= $valor_serv){
					$pdo->query("UPDATE agendamentos SET pago = 'Sim', pgto_parcial = 'Sim' where id = '$id'");
				}else{
					$pdo->query("UPDATE agendamentos SET pgto_parcial = 'Sim' where id = '$id'");
				}
				}			
...
<div class="modal fade" id="modalForm" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static">
	<div class="modal-dialog modal-lg" role="document" style="overflow-y: auto; max-height: 90vh;">
			...
			<form method="post" id="form-text">
				<div class="modal-body">
					...
					<div class="row">
						...
						<div class="col-md-4">						
							<div class="form-group"> 
								<label>Data Retirada <small>(Se for Exame)</small></label> 
								<input maxlength="100" type="text" class="form-control" name="data_retirada" id="data_retirada" placeholder="Ex:08/08/2025 a partir das 14:00">
							</div>						
						</div>
					</div>
					<hr>
...
function editarPac(){
	...
	$.ajax({
        url: 'paginas/' + pag + "/buscar_dados_paciente.php",
        method: 'POST',
        data: {paciente},
        dataType: "html",
