...
$pdo->query("DELETE FROM chamadas WHERE data < curDate()");
	...
	<div class="row">
		...
		<div class="col-md-2" align="center">	
			...
			<div > 				
				...
				<select id="sala" name="sala" class="form-control" onchange="mudarConsultorio()">
					<option value="">Selecionar Consultório</option>
					 <?php 

									$query = $pdo->query("SELECT * from salas where ativo = 'Sim' order by id asc");
								 ...
								 ?>
								  ...
								  <option value="<?php echo $res[$i]['nome'] ?>" <?php if($res[$i]['nome'] == $sala_usuario){ ?> selected <?php } ?>>Consultório: <?php echo $res[$i]['nome'] ?></option>



								<?php } }else{ ?>

									<option value="">Cadastre uma Sala</option>
...
<!-- Modal Relatorio -->
<div class="modal fade" id="modalTriagem" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title" id="exampleModalLabel"><span id="nome_triagem"></span>		

				</h4>
				<button id="btn-fechar-triagem" type="button" class="close" data-dismiss="modal" aria-label="Close" style="margin-top: -25px">
					<span aria-hidden="true">&times;</span>
				</button>
			...
			<div class="modal-body">
				...
				<div id="listar_dados_triagem"></div>

				<div class="row">
                    <div class="col-md-12 mb-2" style="font-weight: normal !important">
                        <label>Hipóteses Diagnósticas / CID-10</label>
                        <input class="form-control" id="hiposteses" name="hiposteses" placeholder="Possíveis Diagnosticos">
                ...
                <div class="row">
                    <div class="col-md-9 mb-2" style="font-weight: normal !important">
                        ...
                        <input class="form-control" id="procedimentos_realizados" name="procedimentos_realizados" placeholder="Sutura, Medicação, Curativo, Exames, etc">
                       
                    </div>

                    <div class="col-md-3 mb-2" style="font-weight: normal !important">
                        <label>Conduta Médica</label>
                         <select class="form-control" name="conduta_medica" id="conduta_medica">  	
                         <option value="">Selecionar Opção</option>	        
					        <option value="Alta Médica">Alta Médica</option>
					        <option value="Encaminhamento Ambulatorial">Encaminhamento Ambulatorial</option>
                ...
                <div class="row">                    
                    <div class="col-md-3 mb-2" style="font-weight: normal !important">
                        <label>Internação</label>
                         <select class="form-control" name="internacao" id="internacao">
                    ...
                    <div class="col-md-3 mb-2" style="font-weight: normal !important">
                        <label>Necessita AIH?</label>
                         <select class="form-control" name="aih" id="aih">
					        <option value="Não">Não</option>
					        <option value="Sim">Sim</option>					        				       
			              </select>                       
                    </div>

                    <div class="col-md-6 mb-2" style="font-weight: normal !important">
                        <label>CID-10 para internação</label>
                        <input class="form-control" id="cid_10" name="cid_10" placeholder="Código Internacional de Doenças - 10ª Revisão">
                ...
                <div class="row">    
                    ...
                    <div class="col-md-6 mb-2" style="font-weight: normal !important">
                        ...
                        <input class="form-control" id="motivo_internacao" name="motivo_internacao" placeholder="">
                       
                    </div>

                    <div class="col-md-6 mb-2" style="font-weight: normal !important">
                        <label>Justificativa Clínica</label>
                        <input class="form-control" id="justificativa_clinica" name="justificativa_clinica" placeholder="">
				...
				<input type="hidden" id="id_triagem" name="id_triagem">				

			</div>	

			<div class="modal-footer">
					<button onclick="$('#btn_pdf_triagem').click();" type="button" class="btn btn-danger">Gerar PDF</button>
					<button id="btn_salvar_triagem" type="submit" class="btn btn-primary">Salvar</button>
				</div>
			</form>
...
<form method="post" action="rel/triagem_class.php" style="display:none" target="_blank">
	<input type="hidden" name="id" id="id_triagem_relatorio">
	<button type="submit" id="btn_pdf_triagem"></button>
</form>
...
<script>
	...
	$("#form-historico").submit(function () {
		 ...
		 let html = $('#historico').trumbowyg('html');
  // Remove <p> tags se for o único elemento
  html = html.replace(/^<p>(.*?)<\/p>$/, '$1');
  $('#historico').trumbowyg('html', html); // atualiza o valor sem <p>
		...
		$.ajax({
			...
			success: function (mensagem) {
				...
				if (mensagem.trim() == "Salvo com Sucesso") {    
					...
					$('#historico').trumbowyg('html', ''); // Limpa o conteúdo do Trumbowyg
...
function mudarConsultorio(){
	var sala = $('#sala').val();

				$.ajax({
			        url: 'paginas/' + pag + "/mudar_sala.php",
			        method: 'POST',
			        data: {sala},
			        dataType: "html",
...
<script>
	$("#form_triagem").submit(function () {		
		$('#btn_salvar_triagem').hide();
		event.preventDefault();		

		var formData = new FormData(this);
		$.ajax({
			url: 'paginas/' + pag + "/salvar_triagem.php",
			type: 'POST',
			data: formData,
			...
			success: function (mensagem) {	
				...
				if (mensagem.trim() == "Salvo com Sucesso") {                  
					 $('#btn-fechar-triagem').click(); 
					 listar();

				} else {
					alertInformativo(msg[0])
				}
				$('#btn_salvar_triagem').show();
			},
			cache: false,
			contentType: false,
			processData: false,
