...
if ($medico != '') {
	$sad = "and medico = '$medico'";
	$sud = "and funcionario = '$medico'";
}else{
	$sad = '';
	$sud = '';
}
...
<table class="table table-striped borda" cellpadding="6">
  ...
  <tbody>
  	...
  	$query = $pdo->query("SELECT * FROM receber where data_pgto >= '$dataInicial' and data_pgto <= '$dataFinal' and referencia = 'Procedimento' and pago = 'Sim' $sad ORDER BY data_pgto asc");
	...
	//totalizar contas recebidas
  	...
  	$query = $pdo->query("SELECT * FROM receber where data_pgto >= '$dataInicial' and data_pgto <= '$dataFinal' and referencia = 'Conta' $sad ORDER BY data_pgto asc");
	...
	//totalizar contas despesas
  	...
  	$query = $pdo->query("SELECT * FROM pagar where data_pgto >= '$dataInicial' and data_pgto <= '$dataFinal' and referencia = 'Conta' $sud  ORDER BY data_pgto asc");
	...
	//totalizar contas pagamentos
  	...
  	$query = $pdo->query("SELECT * FROM pagar where data_pgto >= '$dataInicial' and data_pgto <= '$dataFinal' and referencia = 'Pagamento' and pago = 'Sim' $sud ORDER BY data_pgto asc");
	...
	//totalizar contas despesas
  	...
  	$query = $pdo->query("SELECT * FROM pagar where data_pgto >= '$dataInicial' and data_pgto <= '$dataFinal' and referencia = 'Comissão' and pago = 'Sim' $sud ORDER BY data_pgto asc");
...
<td colspan="3" class="text-danger"> R$ <?php echo $total_saidasF ?></td>
	...
	<div class="col-md-12 p-2">
		...
		<div class="" align="center" style="margin-right: 20px">
			...
			<span class="<?php echo $classe_saldo ?>">R$ <?php echo $saldo_totalF ?></span><br>
...
$query = $pdo->query("SELECT saida, valor FROM receber WHERE data_pgto >= '$dataInicial' AND data_pgto <= '$dataFinal' AND referencia = 'Procedimento' AND pago = 'Sim' $sad ORDER BY data_pgto ASC");

$res = $query->fetchAll(PDO::FETCH_ASSOC);

$total_reg = count($res);

// Inicializa um array para armazenar o total para cada método de pagamento
$total_por_metodo = array();

for ($i = 0; $i < $total_reg; $i++) {
    $metodo_pagamento = $res[$i]['saida'];
    $valor = $res[$i]['valor'];

    // Verifica se o método de pagamento já existe no array
    if (isset($total_por_metodo[$metodo_pagamento])) {
        // Se existir, adiciona o valor ao total existente
        $total_por_metodo[$metodo_pagamento] += $valor;
    } else {
        // Se não existir, cria uma entrada para o método de pagamento
        $total_por_metodo[$metodo_pagamento] = $valor;
    }
...
// Exibindo os totais por método de pagamento
foreach ($total_por_metodo as $metodo => $total) {
  ...
  <thead>
    ...
    <tr align="center" style="text-align: center">
      ...
      <th style="width: 70%" >Metodo de Pagamento (Entradas / Ganhos)</th>      
  ...
  <td  class="text-success"><?php echo $metodo?></td>

<td  class="text-danger"> <?php echo $totalF ?></td>
...
//echo "Total Geral de Serviços: $total_servicos";
 ?>
